...
void TimeManager::init(const Search::LimitsType& limits, Color us, int ply)
{
  ...
  for (int hypMTG = 1; hypMTG <= MaxMTG; ++hypMTG)
  {
      ...
      int hyphMTG = hypMTG / 2;
      int hypMyTimeNoInc =  limits.time[us]
                 + 50 * (hypMTG - hyphMTG - 1)   //guarantees no regression on fishtest!
                 + limits.inc[us] * (hyphMTG)
                 - moveOverhead * (2 + std::min(hypMTG, 40));

      hypMyTime = std::max(hypMyTime, 0);
      hypMyTimeNoInc = std::max(hypMyTimeNoInc, 0);

      int t1 = minThinkingTime + remaining<OptimumTime>(hypMyTime, hypMTG, ply, slowMover);
      int t2 = minThinkingTime + remaining<MaxTime    >(hypMyTimeNoInc, hypMTG, ply, slowMover);
